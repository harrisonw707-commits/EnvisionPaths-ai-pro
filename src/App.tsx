  ...
  const handle2FALogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/login-2fa', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ user_id: tempUserId, token: twoFactorCode })
      });
      const data = await res.json();
      if (res.ok) {
        setSelectedPlan(data.user.plan_type);
                  ...
                  <button 
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (res.ok) {
                          const data = await res.json();
                          console.log('[AUTH] Admin bypass success, sessionId:', data.sessionId);
                          if (data.sessionId) {
                            localStorage.setItem('session_id', data.sessionId);
                          }
                          ...
                          const data = await res.json();
                          console.error('[AUTH] Admin bypass failed:', data.error);
                          setAuthError(data.error || 'Bypass failed');
                        }
                      } catch (e) {
                        console.error('[AUTH] Admin bypass network error:', e);
                ...
                {requires2FA ? (
                  <form onSubmit={handle2FALogin} className="space-y-5">
                    <div className="space-y-2">
                      <label htmlFor="2fa-code" className="text-[10px] font-bold uppercase tracking-widest text-zinc-300 ml-1">Verification Code</label>
                      <div className="relative">
