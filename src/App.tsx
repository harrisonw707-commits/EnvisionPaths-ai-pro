...
Generated by EnvisionPaths Career Intelligence`;
  ...
  const importAllData = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (trimmedText.startsWith('{') || trimmedText.startsWith('[')) {
      ...
      } else if (trimmedText.includes(',') && (trimmedText.toLowerCase().includes('activity') || trimmedText.toLowerCase().includes('email') || trimmedText.toLowerCase().includes('country'))) {
        // Simple CSV Parser for Activity Logs or Google Data
        ...
        const logs = lines.slice(1).map(line => {
          ...
          headers.forEach((header, index) => {
            if (header.includes('email')) log.email = values[index];
            if (header.includes('activity') || header.includes('event')) log.activity = values[index];
            if (header.includes('country')) log.country = values[index];
            if (header.includes('ip')) log.ip_address = values[index];
            if (header.includes('agent')) log.user_agent = values[index];
            if (header.includes('time') || header.includes('date')) log.created_at = values[index];
          });
          return log;
        });
        ...
        throw new Error("Unrecognized file format. Please upload the .json export file or a valid activity CSV.");
      }
  ...
  const downloadSimulationsCSV = () => {
    ...
    const headers = ['ID', 'User ID', 'Job Title', 'Industry', 'Score', 'Status', 'Time'];
        ...
        <AnimatePresence mode="wait">
          {step === 'auth' ? (
            ...
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
              className="max-w-6xl mx-auto mt-12 px-4 pb-20"
            >
              <div className="bg-theme-surface border border-theme p-6 md:p-10 rounded-3xl backdrop-blur-xl shadow-2xl">
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-10">
                  ...
                  <div className="flex gap-4">
                    <label className="px-6 py-3 bg-theme-surface-hover border border-theme text-theme-primary text-xs font-black uppercase tracking-widest rounded-xl hover:opacity-80 transition-all flex items-center gap-2 cursor-pointer disabled:opacity-50">
                    ...
                    <button 
                      ...
                      className="px-6 py-3 bg-theme-surface-hover border border-theme text-theme-primary text-xs font-black uppercase tracking-widest rounded-xl hover:opacity-80 transition-all flex items-center gap-2 disabled:opacity-50"
                    >
                    ...
                    <button 
                      ...
                      className="p-3 bg-theme-surface-hover border border-theme rounded-xl hover:opacity-80 transition-all text-theme-primary disabled:opacity-50"
                    >
                      <RefreshCw size={16} className={isLoadingLogs ? 'animate-spin' : ''} />
                    </button>
                    <button 
                      onClick={() => setStep('setup')}
                      className="px-6 py-3 bg-red-600 text-white text-xs font-black uppercase tracking-widest rounded-xl hover:bg-red-700 transition-all"
                    >
                      Back to App
                    </button>
                  </div>
