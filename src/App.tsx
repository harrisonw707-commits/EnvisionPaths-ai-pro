...
Generated by EnvisionPaths Career Intelligence`;
  ...
  return (
    <div className={`min-h-screen bg-theme-main font-sans text-theme-primary selection:bg-red-600 selection:text-white transition-colors duration-300 ${theme === 'light' ? 'theme-light' : ''}`}>
      ...
      {isLoading && (
        <div
          className="fixed inset-0 bg-theme-main/90 backdrop-blur-xl flex flex-col items-center justify-center z-[100] animate-in fade-in duration-500"
          role="status"
          aria-label="Loading, preparing your session"
          aria-live="polite"
        >
        ...
        <AnimatePresence mode="wait">
          {step === 'auth' ? (
                    ...
                    <div className="bg-theme-surface border border-theme rounded-3xl p-8">
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                        <div className="flex items-center gap-2">
                          ...
                          <h3 className="text-[10px] font-black uppercase tracking-widest text-theme-secondary">Recent Sessions</h3>
                        </div>
                        <div className="flex items-center gap-4">
                          <div className="flex items-center gap-2">
                      ...
                      <div className="grid sm:grid-cols-2 gap-4">
                        {(showAllSessions ? history : history.slice(0, 6)).map((sim) => (
                          <button 
                            key={sim.id} 
                            onClick={() => {
                          ...
                          >
                            <div>
                              ...
                              {sim.industry && (
                                <p className="text-[8px] text-red-500/70 uppercase font-black tracking-widest mt-0.5">{sim.industry}</p>
                              )}
                      ...
                      {history.length > 6 && (
                        <div className="mt-4 text-center">
                          <button
                            ...
                            className="text-[8px] font-black uppercase tracking-widest bg-red-600/10 text-red-500 px-4 py-2 rounded-full border border-red-500/20 hover:bg-red-600 hover:text-white transition-all"
                          >
                            {showAllSessions ? 'Show Less' : `View All ${history.length} Sessions`}
                          </button>
                        </div>
                      )}
                    </div>
            ...
            <motion.div 
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="flex flex-col h-[calc(100dvh-180px)]"
            >
              {/* Interview Header */}
              ...
              <div className="flex-1 overflow-hidden flex flex-col gap-8">
                ...
                {isTyping && (
                  ...
                  <div className="mt-auto pt-6 border-t border-theme">
                    ...
                    <form onSubmit={(e) => handleSendMessage(e)} className="flex flex-col md:flex-row gap-4 items-start md:items-center">
                      ...
                      <div className="relative flex-1 flex flex-col gap-2 w-full">
                        {showRetry && (
                          <button 
                            ...
                            className="self-center px-4 py-2 bg-theme-surface border border-theme rounded-full text-[10px] font-black uppercase tracking-widest text-red-500 shadow-xl flex items-center gap-2 hover:bg-theme-surface-hover transition-all"
                          >
                        ...
                        <textarea
                          ...
                          className="w-full px-4 py-4 bg-theme-surface border border-theme rounded-2xl shadow-lg focus:border-red-600 focus:ring-1 focus:ring-red-600 outline-none transition-all text-base text-theme-primary resize-none overflow-y-auto min-h-[120px] max-h-[240px] leading-relaxed"
                          style={{ height: '120px' }}
                          disabled={isTyping || interactionMode === 'voice'}
                        />
                        <p className="text-[10px] text-theme-secondary text-right px-1">
                          Press <kbd className="px-1 py-0.5 bg-theme-surface-hover border border-theme rounded text-[9px]">Enter</kbd> to send · <kbd className="px-1 py-0.5 bg-theme-surface-hover border border-theme rounded text-[9px]">Shift+Enter</kbd> for new line
                        </p>
                        {/* Desktop send button — inline */}
                        <Tooltip content={isTyping ? "Coach is thinking..." : "Send your response to the coach"}>
                          <button 
                            type="submit"
                            disabled={!input.trim() || isTyping}
                            aria-label="Send message"
                            className="hidden md:flex absolute right-3 top-4 w-12 h-12 bg-red-600 text-white rounded-xl items-center justify-center hover:bg-red-700 disabled:opacity-30 transition-all shadow-lg shadow-red-900/20 border border-red-500/20"
                          >
                            {isTyping ? <RefreshCw size={22} className="animate-spin" /> : <Send size={22} />}
                          </button>
                        </Tooltip>
                        {/* Mobile send button — full-width below textarea */}
                        <button
                          type="submit"
                          disabled={!input.trim() || isTyping}
                          aria-label="Send message"
                          className="md:hidden w-full py-4 bg-red-600 text-white rounded-2xl flex items-center justify-center gap-2 hover:bg-red-700 disabled:opacity-30 transition-all shadow-lg shadow-red-900/20 border border-red-500/20 font-black uppercase tracking-widest text-[10px]"
                        >
