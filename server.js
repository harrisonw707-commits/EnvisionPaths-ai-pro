...
appendFileSync("server_init.log", `[${(/* @__PURE__ */ new Date()).toISOString()}] Server file loaded
`);
...
process.on("uncaughtException", (err) => {
  ...
  logBuffer.push(`[${(/* @__PURE__ */ new Date()).toISOString()}] CRITICAL ERROR: ${err.message}`);
});
process.on("unhandledRejection", (reason, promise) => {
  originalError("[UNHANDLED REJECTION]", reason);
  logBuffer.push(`[${(/* @__PURE__ */ new Date()).toISOString()}] UNHANDLED REJECTION: ${reason}`);
});
console.log = (...args) => {
  const msg = `[${(/* @__PURE__ */ new Date()).toISOString()}] ${args.map((a) => typeof a === "object" ? JSON.stringify(a) : a).join(" ")}`;
  logBuffer.push(msg);
  if (logBuffer.length > 100) logBuffer.shift();
  originalLog.apply(console, args);
};
console.error = (...args) => {
  const msg = `[${(/* @__PURE__ */ new Date()).toISOString()}] ERROR: ${args.map((a) => typeof a === "object" ? JSON.stringify(a) : a).join(" ")}`;
  logBuffer.push(msg);
  if (logBuffer.length > 100) logBuffer.shift();
...
var db = new Database("envision.db", { timeout: 5e3 });
...
async function startServer() {
  ...
  appendFileSync("server_init.log", `[${(/* @__PURE__ */ new Date()).toISOString()}] Initializing test users...
`);
  try {
  ...
  app.post("/api/admin-login", (req, res) => {
    ...
    try {
      ...
      if (email === "harrisonw707@gmail.com") {
        ...
        const fullUser = db.prepare("SELECT id, email, plan_type, is_admin FROM users WHERE id = ?").get(user.id);
