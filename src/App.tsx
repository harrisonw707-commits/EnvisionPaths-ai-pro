...
export default function App() {
  ...
  const [adminTab, setAdminTab] = useState<'overview' | 'stats'>('overview');
...
Generated by EnvisionPaths Career Intelligence`;
  ...
  const fetchActivityLogs = async () => {
    ...
    try {
      ...
      const resSims = await fetch('/api/admin/export-data', { headers: getAuthHeaders() });
      if (resSims.ok) {
                ...
                {isTyping && (
                  ...
                  >
                    <div className="max-w-[85%] flex gap-3 md:gap-4 flex-row">
                      ...
                      <div className="p-4 md:p-6 rounded-2xl bg-theme-surface border border-theme text-theme-primary rounded-tl-none shadow-xl flex flex-col gap-3">
                        ...
                        <p className="text-[9px] font-black uppercase tracking-[0.2em] text-red-500/60 italic">Coach is formulating response...</p>
                      </div>
                    </div>
                  </motion.div>
                )}
                ...
                {adminTab === 'overview' ? (
                  ...
                  <div className="lg:col-span-1 space-y-8">
                      <div className="p-6 bg-theme-input border border-theme rounded-2xl">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <label className="p-4 bg-theme-surface-hover border border-theme rounded-xl hover:opacity-80 transition-all text-center cursor-pointer group">
                            <Upload className="mx-auto mb-2 text-red-500 group-hover:-translate-y-1 transition-transform" size={24} />
                            <span className="text-[10px] font-black uppercase tracking-widest text-theme-primary">Import</span>
                            <input type="file" className="hidden" accept=".json,.csv" onChange={importAllData} />
                          </label>
                          <button 
                          ...
                          >
                            <Download className={`mx-auto mb-2 text-red-500 ${isExporting ? 'animate-bounce' : 'group-hover:translate-y-1 transition-transform'}`} size={24} />
                            <span className="text-[10px] font-black uppercase tracking-widest text-theme-primary">Export</span>
                          </button>
                          <button 
                            ...
                            className="p-4 bg-theme-surface-hover border border-theme rounded-xl hover:opacity-80 transition-all text-center group"
                          >
                            <Globe className={`mx-auto mb-2 text-red-500 ${isImporting ? 'animate-spin' : 'group-hover:scale-110 transition-transform'}`} size={24} />
                            <span className="text-[10px] font-black uppercase tracking-widest text-theme-primary">Sync Play</span>
                          </button>
                          <button 
                          ...
                          >
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-theme-primary">Sync</span>
                          </button>
                          <button 
                            onClick={() => {
                              if (confirm('Are you sure? This will reset all simulation data.')) {
                                fetch('/api/admin/reset-db', { method: 'POST', headers: getAuthHeaders() }).then(() => window.location.reload());
                              }
                            }}
                            className="p-4 bg-red-900/10 border border-red-500/20 rounded-xl hover:bg-red-600 hover:text-white transition-all text-center group"
                          >
                            <Trash2 className="mx-auto mb-2 text-red-500 group-hover:scale-110 transition-transform" size={24} />
                    ...
                    <div className="p-6 bg-theme-input border border-theme rounded-2xl">
                      <div className="flex items-center justify-between mb-6">
                        ...
                        <button 
                          onClick={downloadSimulationsCSV}
                          disabled={simulations.length === 0}
                          className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-theme-secondary hover:text-red-500 transition-colors disabled:opacity-50"
                        >
                          <Download size={14} />
                          CSV
                        </button>
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center justify-between p-3 bg-theme-surface-hover rounded-lg border border-theme">
                          <span className="text-[10px] font-bold uppercase tracking-widest text-theme-secondary">Total Sessions</span>
                          <span className="text-[10px] font-black uppercase tracking-widest text-theme-primary">{simulations.length}</span>
                        </div>
                        <div className="flex items-center justify-between p-3 bg-theme-surface-hover rounded-lg border border-theme">
                          <span className="text-[10px] font-bold uppercase tracking-widest text-theme-secondary">Avg Score</span>
                          <span className="text-[10px] font-black uppercase tracking-widest text-theme-primary">
                            {simulations.length > 0 
                              ? Math.round(simulations.reduce((acc, s) => acc + (s.score || 0), 0) / simulations.length) 
                              : 0}%
                          </span>
                        </div>
                      </div>
                  ...
                  <div className="lg:col-span-2 space-y-8">
                    <div className="p-6 bg-theme-input border border-theme rounded-2xl h-full flex flex-col">
                      <div className="flex items-center gap-4 mb-6">
                        ...
                        <button 
                          ...
                          className="ml-auto flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-theme-secondary hover:text-red-500 transition-colors disabled:opacity-50"
                        >
                          <Download size={14} />
                          Download CSV
                        </button>
                          ...
                          <table className="w-full text-left border-collapse">
                            <thead>
                              <tr className="border-b border-theme">
                                <th className="pb-4 text-[10px] font-black uppercase tracking-widest text-theme-secondary">User</th>
                                <th className="pb-4 text-[10px] font-black uppercase tracking-widest text-theme-secondary">Activity</th>
                                <th className="pb-4 text-[10px] font-black uppercase tracking-widest text-theme-secondary">Country</th>
                                <th className="pb-4 text-[10px] font-black uppercase tracking-widest text-theme-secondary">Time</th>
                              </tr>
                            </thead>
                              ...
                              {isLoadingLogs ? (
                                <tr>
                                  <td colSpan={4} className="py-20 text-center">
                                    ...
                                    <p className="text-[10px] font-black uppercase tracking-widest text-theme-secondary">Loading Logs...</p>
                                  </td>
                                </tr>
                              ) : activityLogs.length === 0 ? (
                                <tr>
                                  <td colSpan={4} className="py-20 text-center text-theme-secondary text-[10px] font-black uppercase tracking-widest">
                                    No activity logs found.
                                  </td>
                                </tr>
                              ) : (
                                activityLogs.map((log) => (
                                  <tr key={log.id} className="hover:bg-theme-surface-hover transition-colors group">
                                    <td className="py-4">
                                      <div className="flex flex-col">
                                        <span className="text-xs font-bold text-theme-primary">{log.email}</span>
                                        <span className="text-[8px] text-theme-secondary font-mono">{log.ip_address}</span>
                                      </div>
                                    </td>
                                    <td className="py-4">
                                      <span className="px-2 py-1 bg-theme-surface border border-theme rounded text-[9px] font-bold text-theme-primary">
                                        {log.activity}
                                      </span>
                                    </td>
                                    <td className="py-4">
                                      <div className="flex items-center gap-2">
                                        <span className="text-[10px] font-black uppercase tracking-widest text-theme-primary">{log.country}</span>
                                      </div>
                                    </td>
                                    <td className="py-4">
                  ...
                  <div className="space-y-12">
                    {/* Statistics View */}
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      <div className="lg:col-span-2 p-8 bg-theme-input border border-theme rounded-3xl">
                        <div className="flex items-center justify-between mb-8">
                          <h3 className="text-xl font-black uppercase italic flex items-center gap-3 text-theme-primary">
                            <BarChartIcon className="text-red-500" size={24} />
                            Installed audience
                          </h3>
                          <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-theme-secondary">
                            <div className="w-3 h-3 bg-blue-800 rounded-sm" />
                            Installed audience (All users, Unique users, Per interval, Daily)
                          </div>
                        </div>
                        <div className="h-[400px] w-full">
                          <ResponsiveContainer width="100%" height="100%">
                            <RechartsBarChart data={getStatsData().chartData}>
                              <CartesianGrid strokeDasharray="3 3" stroke={theme === 'dark' ? '#333' : '#eee'} vertical={false} />
                              <XAxis 
                                dataKey="name" 
                                axisLine={false} 
                                tickLine={false} 
                                tick={{ fill: theme === 'dark' ? '#666' : '#999', fontSize: 10, fontWeight: 'bold' }}
                              />
                              <YAxis 
                                axisLine={false} 
                                tickLine={false} 
                              ...
                              <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                                {getStatsData().chartData.map((entry, index) => (
                                  <Cell key={`cell-${index}`} fill={index === 0 ? '#1e40af' : '#94a3b8'} fillOpacity={index === 0 ? 1 : 0.5} />
                                ))}
                              </Bar>
                            </RechartsBarChart>
                          </ResponsiveContainer>
                      ...
                      <div className="p-8 bg-theme-input border border-theme rounded-3xl">
                        ...
                        <div className="space-y-6">
                          {getStatsData().chartData.map((item, idx) => (
                            <div key={item.name} className="flex items-center justify-between group">
                              <div className="flex items-center gap-3">
                                <span className="text-[10px] font-black text-theme-secondary w-6">{idx + 1}</span>
                                <span className="text-xs font-bold text-theme-primary group-hover:text-red-500 transition-colors">{item.name}</span>
                              </div>
                              <span className="text-xs font-black text-theme-primary">{item.value}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    <div className="p-8 bg-theme-input border border-theme rounded-3xl overflow-hidden">
                      <div className="flex items-center justify-between mb-8">
                        <h3 className="text-xl font-black uppercase italic text-theme-primary">Data table</h3>
                        <div className="flex items-center gap-4 text-[10px] font-black uppercase tracking-widest text-theme-secondary">
                          <span>Show rows: 10</span>
                          <span>1 - 9 of 9</span>
                        </div>
                      </div>
                      <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                          <thead>
                            <tr className="border-b border-theme">
                              <th className="pb-6 text-[10px] font-black uppercase tracking-widest text-theme-secondary">Days</th>
                              <th className="pb-6 text-[10px] font-black uppercase tracking-widest text-theme-secondary">All countries / regions</th>
                              {getStatsData().countries.slice(0, 5).map(country => (
                                <th key={country} className="pb-6 text-[10px] font-black uppercase tracking-widest text-theme-secondary">{country}</th>
                              ))}
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-theme">
                            {getStatsData().tableData.map((row, idx) => (
                              <React.Fragment key={row.date}>
                                <tr className="hover:bg-theme-surface-hover transition-colors">
                                  <td className="py-6 text-xs font-bold text-theme-primary">{row.date}</td>
                                  <td className="py-6 text-xs font-black text-theme-primary">{row.total}</td>
                                  {getStatsData().countries.slice(0, 5).map(country => (
                                    <td key={country} className="py-6 text-xs font-bold text-theme-primary">{row.countries[country] || '-'}</td>
                                  ))}
                                </tr>
                                <tr className="bg-theme-surface-hover/20">
                                  <td className="py-3 text-[8px] font-bold text-theme-secondary uppercase tracking-widest pl-2">Percentage of total</td>
                                  <td className="py-3 text-[8px] font-black text-theme-secondary uppercase tracking-widest">100%</td>
                                  {getStatsData().countries.slice(0, 5).map(country => (
                                    <td key={country} className="py-3 text-[8px] font-black text-theme-secondary uppercase tracking-widest">
                                      {row.total > 0 ? `${Math.round(((row.countries[country] || 0) / row.total) * 10000) / 100}%` : '-'}
                                    </td>
                                  ))}
                                </tr>
                              </React.Fragment>
