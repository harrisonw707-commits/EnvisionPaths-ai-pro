...
Generated by EnvisionPaths Career Intelligence`;
  ...
  const TesterGuide = () => (
    <motion.div 
      ...
      className={`mb-8 p-6 ${theme === 'light' ? 'bg-white border-slate-200 shadow-xl' : 'bg-red-600/10 border-red-600/20 backdrop-blur-xl'} border rounded-3xl relative overflow-hidden group`}
    >
      <div className={`absolute top-0 right-0 p-4 ${theme === 'light' ? 'text-slate-100' : 'opacity-10'} group-hover:opacity-20 transition-opacity`}>
        <Shield size={80} />
      </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <div className="space-y-2">
            ...
            <ul className="space-y-1.5">
              <li className="flex items-center gap-2 text-[10px] text-theme-primary font-bold uppercase tracking-wider">
  ...
  return (
    <div className={`min-h-screen bg-theme-main font-sans text-theme-primary selection:bg-red-600 selection:text-white transition-colors duration-300 ${theme === 'light' ? 'theme-light' : ''}`}>
      ...
      {/* Header */}
      {step !== 'auth' && (
        <header className="fixed top-0 left-0 right-0 z-50 bg-theme-main/80 backdrop-blur-md border-b border-theme px-6 py-4">
